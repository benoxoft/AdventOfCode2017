import {findEvenlyDivisibleValues, parseInputString, calculateChecksumPart2} from "../day2";

const EXAMPLE_SPREADSHEET = `5 9 2 8
9 4 7 3
3 8 6 5`;
...
it("has a result of 4 in this division because in the first row, the only two numbers that evenly divide are 8 and 2", () => {
    expect(findEvenlyDivisibleValues([5, 9, 2, 8],)).toEqual([8, 2]);
    expect(calculateChecksumPart2([[5, 9, 2, 8],])).toBe(4);
});

it("has a result of 3 in this division because in the second row, the two numbers are 9 and 3.", () => {
    expect(findEvenlyDivisibleValues([9, 4, 7, 3],)).toEqual([9, 3]);
    expect(calculateChecksumPart2([[9, 4, 7, 3]])).toBe(3);
});

it("has a result of 2 in the third row", () => {
   expect(findEvenlyDivisibleValues([3, 8, 6, 5],)).toEqual([6, 3]);
   expect(calculateChecksumPart2([[3, 8, 6, 5],])).toBe(2);
});
...
it("has a result 4 + 3 + 2 = 9 in the example", () => {
   expect(calculateChecksumPart2(PARSED_EXAMPLE)).toBe(9);
});

const parsed_data = parseInputString(PUZZLE_INPUT);
const answer = calculateChecksumPart2(parsed_data);
