import {validatePassphrase, validatePassphrase2, validatePassphrases, validatePassphrases2} from "../day4";

const PUZZLE_INPUT: string = `nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
test("aa bb cc dd ee is valid.", () => {
    expect(validatePassphrase("aa bb cc dd ee")).toBeTruthy();
});

test("aa bb cc dd aa is not valid - the word aa appears more than once.", () => {
    expect(validatePassphrase("aa bb cc dd aa")).toBeFalsy();
});

test("aa bb cc dd aaa is valid - aa and aaa count as different words.", () => {
    expect(validatePassphrase("aa bb cc dd aaa")).toBeTruthy();
});

test("abcde fghij is a valid passphrase.", () => {
    expect(validatePassphrase2("abcde fghij")).toBeTruthy();
});

test("abcde xyz ecdab is not valid - the letters from the third word can be rearranged to form the first word.", () => {
    expect(validatePassphrase2("abcde xyz ecdab")).toBeFalsy();
});

test("a ab abc abd abf abj is a valid passphrase, because all letters need to be used when forming another word.", () => {
    expect(validatePassphrase2("a ab abc abd abf abj")).toBeTruthy();
});

test("iiii oiii ooii oooi oooo is valid.", () => {
    expect(validatePassphrase2("iiii oiii ooii oooi oooo")).toBeTruthy();
});

test("oiii ioii iioi iiio is not valid - any of these words can be rearranged to form any other word.", () => {
    expect(validatePassphrase2("oiii ioii iioi iiio")).toBeFalsy();
});


console.log("Answer 1: ", validatePassphrases(PUZZLE_INPUT));
console.log("Answer 2: ", validatePassphrases2(PUZZLE_INPUT));
